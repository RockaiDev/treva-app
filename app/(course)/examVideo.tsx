  ...
  const BuyLesson = async () => {
    ...
    if (lessonData?.price !== undefined) {
      if (+userData.points < +lessonData.price) {
        Alert.alert('المحفظة', 'لا يوجد لديك رصيد كافي لشراء المحاضرة')
        router.push('/(tabs)/Wallet')
        setHasLesson(false)
      } else {
    ...
    <>
      ...
      >
        ...
        <View style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'flex-start', margin: 10, direction: 'rtl' }}>
          <Text style={[ConstantStyles.Title1, { fontSize: 26, textAlign: 'left' }]}>{ExamVideo.title}</Text>
          <Text style={[ConstantStyles.Title2, { fontSize: 18, marginBottom: 5 }]}>تاريخ المحاضرة: {new Date(lessonData.updatedAt).toLocaleDateString()}</Text>
        </View>
          ...
          <View style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent: 'center', direction: 'rtl' }}>
            {ExamVideo.attaches ? (
              ...
              >
                ...
                <Text style={[ConstantStyles.Title2, { fontSize: 18, marginRight: 10 }]}>{ExamVideo.attaches.slice(0, 40)}</Text>
              </TouchableOpacity>
            ) : (
          ...
          <View style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', width: '100%', height: '100%', backgroundColor: 'rgba(0,0,0,0.5)', pointerEvents: !CardBuyLesson ? 'none' : 'auto' }}>
            ...
            >
              <ScrollView
                ...
                style={{ width: '100%', height: '100%', paddingVertical: 120, flex: 1, paddingHorizontal: 10 }}>
                ...
                <View style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', width: '100%', marginTop: 20 }}>
                  <View style={{ display: 'flex', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', width: '100%', direction: 'rtl', marginBottom: 20 }}>
                    <Text style={[ConstantStyles.Title2, { fontSize: 18, marginBottom: 5 }]}>عنوان المحاضرة:</Text>
                    <View style={[ConstantStyles.shadowContainer, { display: 'flex', justifyContent: 'center', alignItems: 'center', backgroundColor: Colors.calmWhite, padding: 5, width: 200, borderRadius: 20, height: 40 }]}>
                  ...
                  {userData.points - lessonData?.price < 0 ? (
                    ...
                    <View style={{ display: 'flex', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', width: '100%', direction: 'rtl', marginBottom: 20 }}>
                      <Text style={[ConstantStyles.Title2, { fontSize: 16, marginBottom: 5 }]}>الرصيد بعد عملية الشراء:</Text>
                      <View style={[ConstantStyles.shadowContainer, { display: 'flex', justifyContent: 'center', alignItems: 'center', backgroundColor: Colors.calmWhite, padding: 5, width: 160, borderRadius: 20, height: 40 }]}>
                ...
                <View style={{ display: 'flex', flexDirection: 'row', justifyContent: 'space-around', alignItems: 'center', width: '100%', marginTop: 20 }}>
                  <TouchableOpacity style={{ backgroundColor: Colors.calmWhite, padding: 10, borderRadius: 5, width: 110, height: 50, display: 'flex', justifyContent: 'center', flexDirection: 'column', alignItems: 'flex-end', direction: 'rtl' }} onPress={() => {
                    setHasLesson(false)
                    setCardBuyLesson(false)
                  }}>
